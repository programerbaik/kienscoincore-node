...
var HTMLCOINRPC = require('kienscoind-rpc');
...
/**
 * Provides a friendly event driven API to kienscoind in Node.js. Manages starting and
 * stopping kienscoind as a child process for application support, as well as connecting
 * to multiple kienscoind processes for server infrastructure. Results are cached in an
...
HTMLCOIN.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'kienscoind/' + name, 'total:', this.subscriptions[name].length);
};
...
HTMLCOIN.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'kienscoind/' + name, 'total:', this.subscriptions[name].length);
};
...
HTMLCOIN.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'kienscoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
HTMLCOIN.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'kienscoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
HTMLCOIN.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in kienscoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in kienscoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in kienscoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './kienscoin.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './kienscoin.conf');
  }

  log.info('Using kienscoin config file:', configPath);
...
HTMLCOIN.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of kienscoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of kienscoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of kienscoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to kienscoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from kienscoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from kienscoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that kienscoind is undergoing a reindex. ' +
             ...
             'of kienscoincore-node services will start.');
    node._reindex = true;
  }
};
...
HTMLCOIN.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/kienscoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/kienscoin.conf';
    }
  }
  return networkPath;
};
...
HTMLCOIN.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('kienscoind/hashblock', message.toString('hex'));
    }
  }
...
HTMLCOIN.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('kienscoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
HTMLCOIN.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('kienscoind/rawtransaction', message.toString('hex'));
    }
...
HTMLCOIN.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/kienscoind.pid';
...
HTMLCOIN.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting kienscoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting kienscoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn kienscoind.'));
      }
...
HTMLCOIN.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to kienscoind.'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, kienscoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
HTMLCOIN.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in kienscoincore is most recent to less recent
 ...
 * @param {Object} txid - A kienscoin transaction id
 * @param {Function} callback
 */
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('kienscoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('kienscoind process did not exit'));
      }
