...
/**
 * Will return the path and default kienscoincore-node configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultConfigFile = path.resolve(defaultPath, './kienscoincore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['kienscoind', 'web'];
