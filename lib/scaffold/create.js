...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'kienscoin-lib': 'git://github.com/HTMLCOIN/kienscoin-lib.git#master',
    'kienscoincore-node': 'git://github.com/HTMLCOIN/kienscoincore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/kienscoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the kienscoincore-node directory and configuration
