...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for kienscoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and kienscoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed kienscoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './kienscoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('HTMLCOINcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in kienscoincore-node services directory
    ...
    if (servicePackage.kienscoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.kienscoincoreNode;
    }
