...
var kienscoind = require('../').services.Bitcoin({
  node: {
    ...
    function(next) {
      ...
      function bitcoindGetBlockNative(deffered) {
        ...
        var hash = fixtureData.blockHashes[c];
          kienscoind.getBlock(hash, function(err, block) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        var hash = fixtureData.txHashes[c];
          kienscoind.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('kienscoind getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('kienscoind getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('kienscoind gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('kienscoind gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
