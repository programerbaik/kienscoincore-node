...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.kienscoind.height === 150) {
            node.services.kienscoind.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.kienscoind.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('kienscoind/hashblock', function(data) {
        bus.unsubscribe('kienscoind/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.kienscoind.height;
      node.services.kienscoind.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.kienscoind.sendTransaction(tx.serialize(), function(err, hash) {
